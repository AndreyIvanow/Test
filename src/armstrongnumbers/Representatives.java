...
public class Representatives implements Iterator<byte[]> {
    ...
    public static Stream<byte[]> asStream(int level, boolean isParallel) {
        Representatives representatives = new Representatives(level);
        return stream(spliteratorUnknownSize(representatives, NONNULL | IMMUTABLE | DISTINCT | CONCURRENT), isParallel);
    }
